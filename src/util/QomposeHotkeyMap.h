 ...
 *
 * We map hotkeys to templated "handler function" values. Our template parameter
 * could be e.g. function pointers, or some other type of values which allows
 * our caller to take some action based on QKeyEvents.
 *
...
class QomposeHotkeyMap
{
	public:
		 ...
		 */
		bool addHotkey(const QomposeHotkey &h, const F &f)
		{
			bool insert = true;
